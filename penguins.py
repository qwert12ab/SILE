...
if page=="简介页面":
    ...
    st.header('数据集介绍')
    st.markdown("""帕尔默群岛企鹅数据集适用于数据探索和数据可视化的一个出色的数据集，也可以作为机器学习入门练习。
                                   该数据集是由Gorman等收集，并发布在一个名为palmerpenguins的R语言包，以对南极企鹅种类进行分类和研究。
                                   该数据集记录了344行观测数据，包含3个不同物种企鹅：阿德利企鹅、巴布亚企鹅和帽带企鹅的各种信息。""")
    st.header('三种企鹅的卡通图像')
    st.image('https://www.hdwallpapersfreedownload.com/uploads/large/animals/penguin-wallpapers-hd.jpg')
elif page == "预测分类页面":
    st.header("预测企鹅分类")
...
if submitted:
    fomat_data_df = pd.DataFrame(data=[format_data],columns=rfc_model.feature_names_in_)
...
with col_logo:
    if not submitted:
        ...
        st.image(f'{prdict_result_species}.png',width=300)
